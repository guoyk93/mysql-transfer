...
mysqldump -h "${MYSQL_SRC_HOST}" "-u${MYSQL_SRC_USER}" "-p${MYSQL_SRC_PASS}" -P "${MYSQL_SRC_PORT}" ${MYSQL_DUMP_OPTS} "${MYSQL_SRC_NAME}" > dump.sql

mysql -h "${MYSQL_DST_HOST}" "-u${MYSQL_DST_USER}" "-p${MYSQL_DST_PASS}" -P "${MYSQL_DST_PORT}" ${MYSQL_LOAD_OPTS} "${MYSQL_DST_NAME}" < dump.sql
